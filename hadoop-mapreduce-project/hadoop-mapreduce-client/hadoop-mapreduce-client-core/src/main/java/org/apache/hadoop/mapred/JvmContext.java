...
class JvmContext implements Writable {
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    // Try reading JvmContext assuming JVMId uses int for id.
    JvmContext jvmContext = readFieldsInternal(inBytes, false);
    ...
    if (jvmContext == null || !jvmContext.pid.equals(HARDCODED_PID)) {
      // Looks like reading it as int didn't work. Fall back to reading as long.
      jvmContext = readFieldsInternal(inBytes, true);
      if (LOG.isDebugEnabled()) {
      ...
      if (jvmContext == null || !jvmContext.pid.equals(HARDCODED_PID)) {
        // Reading as long didn't work either. Give up!
