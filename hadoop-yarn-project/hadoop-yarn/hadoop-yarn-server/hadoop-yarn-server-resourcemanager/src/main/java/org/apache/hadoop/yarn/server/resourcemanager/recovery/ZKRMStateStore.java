...
public class ZKRMStateStore extends RMStateStore {
  ...
  protected List<ACL> constructZkRootNodeACL(
    ...
    zkRootNodeUsername =
        HAUtil.getConfValueForRMInstance(YarnConfiguration.RM_ADDRESS, conf);
  ...
  public synchronized void initInternal(Configuration conf) throws Exception {
    ...
    if (HAUtil.isHAEnabled(conf)) {
      String zkRootNodeAclConf = HAUtil.getZKStoreRootNodeACLString(conf);
  ...
  private void debugLogRootNodeAcls(String prefix) throws KeeperException,
  ...
  private synchronized void fence() throws Exception {
    if (LOG.isDebugEnabled()) {
      debugLogRootNodeAcls("Before fencing\n");
    }
    ...
    if (LOG.isDebugEnabled()) {
      debugLogRootNodeAcls("After fencing\n");
    }
  }
