...
public class TestDatanodeStartupFixesLegacyStorageIDs {
  ...
  /**
   * Startup from a 2.5-layout that has legacy storage IDs correctly
   * generates new storage IDs. Note that the upstream test case used
   * data dirs that had a 2.6-layout so we had to re-create 2.5-layout
   * for CDH5.3.x.
   * Test case for HDFS-7575.
   */
  @Test (timeout=300000)
  public void testUpgradeFrom22via25FixesStorageIDs() throws IOException {
    runLayoutUpgradeTest(GenericTestUtils.getMethodName(), null);
  }

  /**
   * Startup from a 2.5-layout that already has unique storage IDs does
   * not regenerate the storage IDs.
   * Test case for HDFS-7575.
   */
  @Test (timeout=300000)
  public void testUpgradeFrom25PreservesStorageIDs() throws IOException {
    ...
    runLayoutUpgradeTest(GenericTestUtils.getMethodName(),
                         "DS-bd47efcb-f1e9-4678-9273-de55d9be1e11");
  }
}
