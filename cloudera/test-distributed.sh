...
docker run -v "$(pwd):/hadoop" -w /hadoop docker-registry.infra.cloudera.com/cauldron/ubuntu1604:1505254507 bash -c "adduser --uid $(id -u) --gecos '' --disabled-password pre_post_commit; su pre_post_commit -c '/hadoop/cloudera/inside-docker.sh'"